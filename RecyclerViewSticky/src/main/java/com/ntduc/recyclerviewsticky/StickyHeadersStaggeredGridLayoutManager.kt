...
class StickyHeadersStaggeredGridLayoutManager<T> : StaggeredGridLayoutManager
  ...
  private val mHeaderPositionsObserver: RecyclerView.AdapterDataObserver =
    HeaderPositionsAdapterDataObserver()
  
  // Sticky header's ViewHolder and dirty state.
  private var mStickyHeader: View? = null
  private var mStickyHeaderPosition: Int = RecyclerView.NO_POSITION
  ...
  override fun computeVerticalScrollExtent(state: RecyclerView.State): Int {
    detachStickyHeader()
    val extent: Int = super.computeVerticalScrollExtent(state)
    attachStickyHeader()
    return extent
  }
  ...
  override fun computeVerticalScrollOffset(state: RecyclerView.State): Int {
    detachStickyHeader()
    val offset: Int = super.computeVerticalScrollOffset(state)
    attachStickyHeader()
    return offset
  }
  ...
  override fun computeVerticalScrollRange(state: RecyclerView.State): Int {
    detachStickyHeader()
    val range: Int = super.computeVerticalScrollRange(state)
    attachStickyHeader()
    return range
  }
  ...
  override fun computeHorizontalScrollExtent(state: RecyclerView.State): Int {
    detachStickyHeader()
    val extent: Int = super.computeHorizontalScrollExtent(state)
    attachStickyHeader()
    return extent
  }
  ...
  override fun computeHorizontalScrollOffset(state: RecyclerView.State): Int {
    detachStickyHeader()
    val offset: Int = super.computeHorizontalScrollOffset(state)
    attachStickyHeader()
    return offset
  }
  ...
  override fun computeHorizontalScrollRange(state: RecyclerView.State): Int {
    detachStickyHeader()
    val range: Int = super.computeHorizontalScrollRange(state)
    attachStickyHeader()
    return range
  }
  ...
  override fun computeScrollVectorForPosition(targetPosition: Int): PointF? {
    detachStickyHeader()
    val vector: PointF? = super.computeScrollVectorForPosition(targetPosition)
    attachStickyHeader()
    return vector
  }
    ...
    if (headerCount > 0 && childCount > 0) {
      ...
      if (anchorView != null && anchorPos != -1) {
        ...
        if (headerPos != -1 && (headerPos != anchorPos || isViewOnBoundary(anchorView))
          ...
          if (layout || (mStickyHeader != null && getPosition(mStickyHeader!!) != headerPos)) {
   ...
   * [.mStickyHeader].
   */
  private fun createStickyHeader(@NonNull recycler: RecyclerView.Recycler, position: Int) {
  ...
  private fun bindStickyHeader(@NonNull recycler: RecyclerView.Recycler, position: Int) {
    ...
    if (mStickyHeader != null && mPendingScrollPosition != RecyclerView.NO_POSITION) {
  ...
  private fun scrapStickyHeader(recycler: RecyclerView.Recycler?) {
    ...
    if (stickyHeader != null && mAdapter is StickyHeaders.ViewSetup) {
    ...
    if (recycler != null && stickyHeader != null) {
    ...
    return if (orientation == VERTICAL) {
      ...
      if (nextHeaderView != null) {
        y = if (reverseLayout) {
          ...
          (nextHeaderView.top - headerView.height).toFloat().coerceAtMost(y)
        }
      }
      y
    } else {
    ...
    return if (orientation != VERTICAL) {
      ...
      if (nextHeaderView != null) {
        x = if (reverseLayout) {
          ...
          (nextHeaderView.left - headerView.width).toFloat().coerceAtMost(x)
        }
      }
      x
    } else {
  ...
  private inner class HeaderPositionsAdapterDataObserver : RecyclerView.AdapterDataObserver() {
    override fun onChanged() {
      ...
      mHeaderPositions.clear()
      val itemCount: Int = mAdapter?.itemCount ?: 0
    ...
    override fun onItemRangeInserted(positionStart: Int, itemCount: Int) {
      ...
      for (i in positionStart until positionStart + itemCount) {
        if (mAdapter != null && mAdapter!!.isStickyHeader(i)) {
  ...
  class SavedState : Parcelable {
    internal var superState: Parcelable? = null
    internal var pendingScrollPosition = 0
