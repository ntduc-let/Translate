package com.ntduc.android.apps.translate.utils.file
...
import com.ntduc.android.apps.translate.data.dto.file.BaseAudio
import com.ntduc.android.apps.translate.data.dto.file.BaseFile
import com.ntduc.android.apps.translate.data.dto.file.BaseImage
import com.ntduc.android.apps.translate.data.dto.file.BaseVideo
import java.io.*
import java.util.*
...
fun getFilesBySequentially(
    directoryPath: String = Environment.getExternalStorageDirectory().path,
    types: List<String> = listOf(*FileType.ALL)
): List<BaseFile> {
    val baseFiles = arrayListOf<BaseFile>()
    val folderStack = Stack<File>()
    folderStack.add(File(directoryPath))

    while (!folderStack.empty()) {
        val files = folderStack.pop().listFiles()
        if (files != null) {
            for (file in files) {
                if (file.isDirectory) {
